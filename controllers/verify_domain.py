...
from helpers import auth_check, common_strings, logging_setup, queue_to_db
"""
API Call: POST
Endpoint: https://{url}/v2/verify-domain?force=true
Body: {
        "value": "www.associatedlabor.com"
      }
...
logger = logging_setup.initialize(common_strings.strings['domain-verify'], 'logs/domain_verify_api.log')


class VerifyDomain(Resource):
    ...
    def post():
        ...
        logger.debug(f"domain checker request received for {value}")
        ...
        check = utils.check_force(value, force, collection=common_strings.strings['domain-verify'],
        ...
        # if database has an entry with results and force is false, send it
            ...
            logger.debug(f"domain checker response sent for {value} from database lookup")
            ...
            utils.mark_db_request(value, status=common_strings.strings['status_queued'],
                                  collection=common_strings.strings['domain-verify'])
            output = {common_strings.strings['key_value']: value, common_strings.strings['key_ip']: ip}
        output['domain_verified'] = domain_verified
        queue_to_db.domainverify_response_db_addition(value, output)
        return output, 200
